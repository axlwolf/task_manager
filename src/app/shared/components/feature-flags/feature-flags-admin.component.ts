      ...
      <div class="space-y-3">
        ...
        >
          <span class="font-medium text-gray-700">{{
            formatFlagName(flag[0])
          }}</span>
          <div class="flex items-center">
            <span
              class="mr-2 text-sm"
              [class.text-green-600]="flag[1]"
              [class.text-red-600]="!flag[1]"
            >
...
export class FeatureFlagsAdminComponent implements OnInit {
  ...
  allFlags = computed(() => {
    const flags = this.featureFlagService.allFlags();
    return Object.entries(flags).sort((a, b) => a[0].localeCompare(b[0]));
  });
