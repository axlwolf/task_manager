...
describe('DialogService', () => {
  ...
  beforeEach(() => {
    // Create spies
    dialogInstance = jasmine.createSpyObj('DialogComponent', 
      ['open'], 
      {
  ...
  it('should open a dialog with default config', fakeAsync(() => {
    ...
    // Assert
    expect(viewContainerRef.createComponent).toHaveBeenCalledWith(DialogComponent);
    expect(dialogComponentRef.location.nativeElement.querySelector).toHaveBeenCalledWith('.dialog-body');
  ...
  it('should configure dialog with provided options', fakeAsync(() => {
    ...
    expect(dialogInstance.closeOnBackdropClick).toBe(config.closeOnBackdropClick);
  }));

  it('should handle dialog close event', fakeAsync(() => {
    // Arrange
    const dialogRef = service.open(TestComponent, {}, viewContainerRef);
    const closeCallback = (dialogInstance.dialogClose.subscribe as jasmine.Spy).calls.mostRecent().args[0];
  ...
  it('should handle dialog confirm event', fakeAsync(() => {
    // Arrange
    const dialogRef = service.open(TestComponent, {}, viewContainerRef);
    const confirmCallback = (dialogInstance.dialogConfirm.subscribe as jasmine.Spy).calls.mostRecent().args[0];
